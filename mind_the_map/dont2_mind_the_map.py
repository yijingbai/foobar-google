...
class Muni(object):
  def __init__(self, subway, skipper = None):
    self.subway = subway
  ...
  def get_paths(self):
    for dir_count in xrange(1, 1 + self.direction_max):
      for route in product(xrange(self.direction_max), repeat=dir_count):
        yield route

  def step(self, station_from, direction):
    station_to = self.subway[station_from][direction]
    if self.skipper == station_to:
      station_to = self.subway[station_to][direction]
    if self.skipper == station_to:
      station_to = station_from
    return station_to

  def final_destination(self, station, path):
    for direction in path:
      station = self.step(station, direction)
    return station

  def path_fail_fast(self, path):
    if self.skipper is None:
      crab_station = 0
    else:
      crab_station = (self.skipper + 1) % self.station_max
    meet_point = self.final_destination(crab_station, path)
    for origin_station in xrange(0, self.station_max):
      if self.skipper == origin_station:
        continue
      last_station = self.final_destination(origin_station, path)
      if last_station != meet_point:
        return -3
    return -1

  def try_all_paths(self):
    for path in self.get_paths():
      if -1 == self.path_fail_fast(path):
        return -1
    return -3

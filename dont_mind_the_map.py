...
# http://stackoverflow.com/questions/28601864/hints-for-puzzle-pls-dont-mind-the-map
# https://github.com/rtheunissen/foobar/blob/master/dont_mind_the_gap.py
...
def answer(subway):
  ...
  if res_ah == -1:
    return -1
  for deserter in xrange(min(5, len(subway))):
    res_ab = Enthiran(subway)
    res_ab.deserter = deserter
    res = res_ab.converge()
    if res == -1:
      return deserter
  return -2
...
class Enthiran(object):
  ...
  def __init__(self, subway_system):
    ...
    self.giants = set() 
    self.deserter = None # station number that is closed down, (or to detour around)

  def _step(self, giant, direction):
    '''Return a new giant, transformed after stepping forward
    '''
    next_robots = [self.subway[r][direction] for r in giant]
    return frozenset(next_robots)
  ...
  def _step_detour(self, giant, direction):
    ...
    if self.deserter is not None:
      ...
      for station_from in giant:
        ...
        next_robots.append(station_to)
      return frozenset(next_robots)
    else:
      return self._step(giant, direction)
  ...
  def _run(self, giant):
    ...
    for direction in xrange(self.direction_max):
      ...
      if len(next_giant) == 1:
        return -1
      elif next_giant in self.giants:
        continue
      else:
        self.giants.add(next_giant)
  ...
  def converge(self):
    all_robots = range(min(5, self.robot_max))
    if self.deserter is not None:
      all_robots.remove(self.deserter)
    first_giant = frozenset(all_robots)
...
def steptest(subway, giant, direction, truth):
  ah = Enthiran(subway)
  next_giant = ah._step(giant, direction)
...
def steptests():
  ...
  truth = frozenset([1, 2])
  direction = 0
  steptest(subway, first_giant, 0, truth)
...
# ========================= Submitted ======================
# foobar:~/dont_mind_the_map USER$ submit solution.py
# Are you sure you want to submit your solution?
# [Y]es or [N]o: Y
# Submitting solution...
# Submission: SUCCESSFUL. Completed in: 4 days, 14 hrs, 43 mins, 13 secs.

# Current level: 5. Challenges to complete level: 4.  
